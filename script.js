...
let today = new Date();
let monthL = new Date().toLocaleString("en-en", { month: "long" });
...
function changeImg() {
	...
	if (activeUser.img != "noImg") {
		...
		userImgHeader.innerHTML = `<div class="noImg" id="noImg">${firstLetter(
			activeUser.name
		)}</div>`;
		userImg.innerHTML = `<div class="noImg" id="noImg" style="width: 5rem; height: 5rem;">${firstLetter(
			activeUser.name
		)}</div>`;
	}
}
...
function clearAllDivs() {
	...
	let awaitingFeedbackSummary = document.getElementById(
...
function sayHello() {
	...
	container.innerHTML += `Good Morning ${
		activeUser.name
	}. It's ${monthL} ${today.getDate()}, ${today.getFullYear()}. Have a nice Day.`;
	container.style =
		"background: var(--leftGrey); padding: 4rem 1rem 1rem 1rem; width: 80%;";
	container.classList.remove("hide");
